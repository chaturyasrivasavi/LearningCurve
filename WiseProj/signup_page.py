...
class SignUpPage:
    ...
    def signup(self):
        ...
        match1 = re.search(r'[0-9][0-9]B0\w', username) or re.search(r'[0-9][0-9]b0\w', username)
        ...
        if match1 and match1.start() == 0 and len(username) == 10 and len(password) >= 8 and password == confirm:
            self.cursor.execute("INSERT INTO users (username, password) VALUES (%s, %s)", (username, password))
            self.db.commit()
            messagebox.showinfo('Signup', 'Successfully Signed Up')
        else:
            messagebox.showerror('Invalid','Invalid Username or Passwords does not match')
